	...
	&
		\begin{tabular}{lcll}
				$\bnfpn{E}$ & \deriv & $\bnfpn{E} \bnfts{*} \bnfpn{E}$ \\
				  ...
				  & \deriv & $\bnfts{id} \bnfts{+} \bnfpn{E} \bnfts{*} \bnfpn{E}$ \\
				  & \deriv & $\bnfts{id} \bnfts{+} \bnfts{id} \bnfts{*} \bnfpn{E}$ \\
				  & \deriv & $\bnfts{id} \bnfts{+} \bnfts{id} \bnfts{*} \tok{id}$
		\end{tabular}
...
\begin{frame}[background=8]{Algorithm of FOLLOW}
	...
	\item To compute FOLLOW($A$) for a nonterminal $A$, apply the following rules until nothing can be added to any FOLLOW set
		\begin{enumerate}
		\vfill
		\item Place \tok{eof} in FOLLOW($S$), where $S$ is the start symbol, and \tok{eof} is the input right end-marker
		\vfill
	...
	\begin{columns}
		...
		\begin{column}{.5\linewidth}
			\begin{tabular}{@{}ll@{}}
				E & \deriv E \tok+ E \\
				& \deriv E \tok+ E \tok+ E \\
				& \deriv E \tok+ E \tok+ E \tok+ E \\
				& \deriv E \tok+ E \tok+ E \tok+ E \tok+ E \\
				& \deriv \dots
			\end{tabular}
		\end{column}
	\end{columns}	
...
\begin{leftlawnframe}{Parsing a $LL(1)$ Grammar}{ll1}
		...
		Table $M[A,a]$ is built from FIRST and FOLLOW sets, where $A$ is a nonterminal, and $a$ is a terminal or \tok{eof}
\end{leftlawnframe}
		...
		\Begin{
			\ForEach{$\left(\bnfpn{A} \bnfopo \alpha\right) \in P$}{
				...
				\If{$\bnfes \in$ FIRST($\alpha$)}{
					...
					\If{\tok{eof} $\in$ FOLLOW($A$)}{
						$M[A,\tok{eof}]$ \affect $M[A,\tok{eof}] \cup \left(\bnfpn{A} \bnfopo \alpha\right)$
					}
				}
			}
...
\begin{frame}[t]{Example of Table Building}
	...
	\begin{tabularx}{\linewidth}{|c|X|X|X|X|X|X|}
		\hline
		\tabularheading&\chead{$\bnfts{id}$}&\chead{$\bnfts{+}$}&\chead{$\bnfts{*}$}&\chead{$\bnfts{(}$}&\chead{$\bnfts{)}$}&\chead{\tok{eof}} \\
		\hline
	...
	\putat(150,200){\parbox[t]{.55\paperwidth}{\normalfont\normalcolor\small
		\only<2-3>{	\Emph{For production:} \bnfmark{1}~$\bnfpn{E} \bnfopo \bnfpn{T} \bnfpn{E'}$ \\
				...
				\only<3>{Then put the production in $M[E,\tok(]$ and $M[E,\tok{id}]$; the rest of the line is \emph{error}}
		}
		...
		\only<5,6>{	\Emph{For production:} \bnfmark{3}~$\bnfpn{E'} \bnfopo \bnfes$ \\
				FIRST($\bnfes$) = $\{ \bnfes \}$ \\
				FOLLOW($E'$) = $\{ \bnfts{)}, \tok{eof} \}$ \\
				Put the production in $M[E',\tok)]$ \\
				\only<6>{Put the production in $M[E',\tok{eof}]$}
		}
		...
		\only<9>{	\Emph{For production:} \bnfmark{6}~$\bnfpn{T'} \bnfopo \bnfes$ \\
				FIRST($\bnfes$) = $\{ \bnfes \}$ \\
				FOLLOW($T'$) = $\{ \bnfts{+}, \bnfts{)}, \tok{eof} \}$ \\
				Put the production in $M[T',\bnfts{+}]$ and $M[T,\bnfts{)}]$ \\
				Put the production in $M[T',\tok{eof}]$
		}
...
\begin{frame}[t]{Example of Reductions}
	...
	\item A possible sequence of reductions is: \\
		{$\bnfts{id}~\bnfts{*}~\bnfts{id} \reduce~F~\tok*~\tok{id} \reduce~F~\bnfts{*}~F \reduce~F~\bnfts{*}~F~\bnfes \reduce~F~\bnfts{*}~F~T' \reduce~F~T'$} \\
		...
		\Begin{
			...
			\While{$f \neq S$}{
				\uIf{$\exists h \in f | f = \alpha\;h\;\beta\;;\,\beta$ contains only terminals}{
					...
					\Else{
						\throw{\ccode{"Cannot find a production for reduction"}}\;
					}
				}
				\Else{
					\throw{\ccode{"Cannot find a handle"}}\;
				}
			}
			\Return $d$\;
		}
...
\begin{frame}{Conflicts During Shift-Reduce Parsing}
	...
	\begin{center}
		\simplebox*[.4\linewidth]{
			\emph{Shift/reduce Conflict} \\
			Cannot decide whether to shift or to reduce
		}
		\hspace{.5cm}
		\simplebox*[.4\linewidth]{
			\emph{Reduce/reduce Conflict} \\
...
\begin{frame}[background=9]{Example of Shift-Reduce Conflict}	
	...
	\item Consider the stack : $\tok{eof} \dots \bnfts{if} \bnfpn{expression} \bnfts{then} \bnfpn{statement}$
	\item Consider the input : $\bnfts{else} \dots \tok{eof}$
	\vfill
...
\begin{frame}[t]{Example of Reduce-Reduce Conflict}	
	...
	\item Consider the grammar with array indexes between parenthesis:
		...
		\end{bnf}\end{footnotesize}
	\vspace{-.75cm}
	\item Consider the stack : $\tok{eof} \dots \bnfts{id} \bnfts{(} \bnfts{id} \bnfts{)}$
	\item Consider the input : $\bnfts{,} \bnfts{id} \bnfts{)} \dots \tok{eof}$
...
\begin{frame}[background=9]{ACTION in the $LR$-Parsing Table}
	\begin{itemize}
	\item This function takes as arguments a state $s_i$ and a terminal $\bnfts{a}$ (or \tok{eof}).
	\vfill
...
\begin{frame}[t]{Algorithm for $LR$-Parsing}
	...
	\Begin{
		...
		\While{$\neg stopParser$}{
			...
			\Else{
				\throw~\ccode{"No production found"} \;
			}
		}
	}
	\end{myalgorithm}
...
\begin{frame}{Define the Regular Expression Productions}
	\begin{definition}
		\ccode{[{\textless}state\_list{\textgreater}] {\textless}kind{\textgreater} [\tok{IGNORE\_CASE}] \tok{:}} \\
...
\begin{frame}[t]{BNF Productions}
	\begin{definition}\small
		...
		\ccode{{\textless}identifier{\textgreater} \tok( {\textless}parameters{\textgreater} \tok) \tok:} \\
		\ccode{{\textless}java\_block{\textgreater}} \\
		\ccode{\tok\{ {\textless}expansion\_choices{\textgreater} \tok\}}
	\end{definition}
	\begin{itemize}
