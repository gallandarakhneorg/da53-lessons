...
\begin{frame}[background=6]{{What are the Functions} FIRST and FOLLOW?}
	\begin{itemize}
	\item The construction of both top-down and bottom-up parsers uses 0two functions associated with a grammar G:
		\begin{enumerate}
		\item FIRST
		\item FOLLOW
		\end{enumerate}
...
\begin{frame}{Nonrecursive Predictive Parsing}
	...
	\item Parser mimics a leftmost derivation. If $w$ is the input that has been matched so far, then the stack holds a sequence of grammar symbols a such that:
		\begin{center}
			$S \seqderiv w\;\alpha$
		\end{center}
...
\begin{frame}[t]{Example of CLOSURE}
	...
	\only<3>{Consider $E$-productions, $T$-productions and $F$-productions because they are both on the right of the dot. Items for $E$-productions are already inside $I_1$, but not items for $T$-productions. \\}
\end{frame}
