...
do {
  i = i + 1;
	...
	\end{columns}
	\vspace{.5cm}
	\hiconbox{Variable declarations are to translated\newline
		Memory size of a byte value is 8\newline
		Address is the number of a valid storage cell}{information-icon}
\end{frame}
...
\begin{frame}{{Do we need more} three-address instructions?}
	\begin{rightarrowsequence}
		...
		\arrow[bg=CIADgreen]{
			TAC Instructions close to machine instructions $\Rightarrow$ easier to implement intermediate form on target machine
		}
...
\begin{frame}{$\phi$-Function}
	...
	\alertbox*{$\phi$-function replies the ``defined'' value from its arguments}
\end{frame}
