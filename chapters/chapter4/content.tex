...
\begin{frame}{Synthetized and Inherited Attributes}
	...
	\end{definitionblock}
	\alertbox{Synthetized attribute is defined by a semantic rule}
	\vspace{.5cm}
	...
	\end{definitionblock}
	\alertbox{Inherited attribute is read in a semantic rule}
\end{frame}
...
\begin{frame}{Evaluating a SDD with a Parse Tree}
	...
	\end{description}
	\vspace{.5cm}
	\hiconbox{Parse tree $\neq$ Syntax tree (source program representation, see slides later)}{problem-icon}
	\vspace{.5cm}
...
\begin{frame}[t]{{Problem to Determine the Order} of the Attributes}
	...
	\vspace{.25cm}
	\hiconbox{Given a SDD, it is very hard to cycle}{information-icon}
	\vspace{.25cm}
	\hiconbox{
...
\begin{frame}{Implementation of Three-Address Code}
	\alertbox*{Three-address instructions could be implemented in a compiler as \emph{objects} or as \emph{records} following one of:}
	\vspace{.25cm}
...
\begin{frame}{$\phi$-Function}
	...
	\alertbox{Impossible to determine which \tactext{x} value is used for \tactext{x * a} \newline
		$\Rightarrow$ harder to optimize the target code}
...
\begin{frame}[t]{Definition of Records or Classes}
	...
	\begin{sdd}
		...
		\sddalt{\sddts{record} \; \sddts{\{} \; \sddpn{Types} \; \sddts{\}}}{
			head.type = record; head.with = Types.width}
...
\begin{frame}{{First Form:} Type Synthesis}
	...
	\item Relationship between types must be defined, e.g., $T_1 \subset T_2$ for value set of $T_A$ are included in value set of $T_2$
	\end{itemize}
	\vspace{.5cm}
	\begin{example}
		\begin{itemize}
