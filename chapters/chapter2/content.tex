...
\begin{frame}{Token}
	\begin{definition}
		...
		\begin{itemize}
			...
			\item token name is written in \tok{bold-face}
		\end{itemize}
	\end{definition}
	\vfill
	\begin{example}
		\begin{itemize}
		\item Let the statement: \code{printf("Total = \%d\n", score);}
		\item both \code{printf} and \code{score} are lexemes matching the pattern for token \tok{id}
		\end{itemize}
	\end{example}
\end{frame}
...
\begin{frame}{Attribute of Token}
	...
	\begin{examples}
		\begin{itemize}
		\item value of the parsed number (lexeme) for token \tok{number}
		\item position of the identifier into the symbol table for token \tok{id}
		\end{itemize}
	\end{examples}
	\vspace{.5cm}
...
\begin{frame}{Relation between the Lexical Analyzer and the Parser}
	...
	\vspace{.5cm}
	\begin{center}
		\includegraphics[width=.8\linewidth]{lexical_parser_relation}
	\end{center}
	\vspace{.5cm}
	\begin{itemize}
...
\begin{frame}{Reading the Source Program}
	...
	\vspace{.25cm}
	\begin{example}
		To be sure that a character is the last of an identifier, the next character must be read, and it is not part of the lexeme for \tok{id}
	\end{example}
\end{frame}
...
\begin{frame}{Principles of the Two-Buffer Scheme}
	...
	\begin{description}
		...
		\item \tok{eof} character is put in the buffer when there is not enough characters in the input
	\end{description}
	\vspace{1cm}
...
\begin{frame}{{Be More Efficient} with Sentinels}
	...
	\end{alertblock}
	\vspace{.5cm}
	$\Rightarrow$ To improve the speed of the treatment, we can combine the two tests by extending each buffer with a \emph{sentinel character} (usually \tok{eof}) \\
	\vspace{.5cm}
	\begin{center}
...
\begin{frame}{{Can We Run Out Of} Buffer Space?}
	\begin{leftarrowsequence}
		...
		\arrow[bg=CIADgreen, decoration=icon-solution]{
			Reply a sequence of \tok{str} tokens, one for each of the shorter strings (see example)
		}
	\end{leftarrowsequence}
	\begin{example}
...
\begin{frame}{{Solving the Problem} with Identifiers and Keywords \insertcontinuationtext}
	\simplebox{
		\raggedright\begin{tabularx}{\linewidth}{@{}lX}
			...
			\begin{itemize}
				\item tokens must be prioritized so that the reserved-word tokens are recognized in preference to \tok{id}
...
\begin{frame}[t,fragile]{Example of Code for the Token \tok{relop}}
	...
	\simplebox{
		\raggedright\begin{tabularx}{\linewidth}{@{}lX}
			...
			\begin{itemize}
				...
				\item We have only to use them \emph{before} we use the diagram for \tok{id}, in order the keywords to be reserved words
			\end{itemize}
		\end{tabularx}
	}
\end{frame}
