...
Auxiliary functions
	...
	\only<3>{\begin{block}{Auxiliary functions}
		\begin{itemize}
		\item Holds additional functions that are used in the actions
		\end{itemize}
	\end{block}}
\end{frame}
...
\begin{frame}[background=6]{Lexical Analyzer by Hand}
	To go deeper in how a program like Lex turns its input program into a sequence of tokens, the formalism called ``\Emph{finite automata}'' is at the heart of this transition
\end{frame}
...
\begin{frame}[fragile,background=6]{Algorithm for Converting NFA to DFA}
	\begin{myalgorithm}
		...
		\Behavior{\begin{enumerate}
			...
			\item DFA may be built from the table $Dtran$
			\end{enumerate}}
	\end{myalgorithm}
\end{frame}
...
\begin{frame}[t]{Example of Simulation of DFA}
	...
	\putat(0,25){\parbox[t]{.9\linewidth}{\normalsize\mdseries\normalcolor
		...
		\only<4>{	
				...
				Read \texttt{"b"} \\[-.5em]
				Pass the edge of the DFA, and update the current state \\[-.5em]
				Because the state $8$ is a final state in the NFA, the state $(8)$ in DFA is marked
				\end{footnotesize}
		}
		\only<5>{	
				\begin{footnotesize}
